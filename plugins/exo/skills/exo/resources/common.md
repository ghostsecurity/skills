...
Shared by all three intents: the read primitives for walking runs, the worker capability rules, the DTO discovery discipline, the write primitives for changing resources, the runtime contract every step runs under, the unprobeable nodes, the in-bundle invocation note, and the production and secret safety rules. Read this before any intent recipe.
...
- `get_workflow_memory(workflow_id, table?)` returns a workflow's stored memory snapshot: every table with its columns and row count, plus one page of rows when `table` is set. `clear_workflow_memory(workflow_id)` deletes the snapshot, so the next run starts empty. It fails while the workflow has active runs.
...
| Target | Write path |
|---|---|
| Credential | Reuse by ID where one fits. Otherwise create through the discovered DTO. New secret values pass through the LLM context, so warn in the same message that requests the paste. Prefer having the user set the value in the UI. The create rejects an environment token credential with an empty `token`, so create it with the placeholder `REPLACE_IN_UI`. Then ask the user to replace it in the UI, and wait for their confirmation before the first run. |
...
| Task | Create or update through the discovered DTO. The execution mode lives here, and metric definitions live on the nested `task_metric` resource. Set exactly one of `instruction`, which pairs an agent with a model, or `entrypoint`, where the runner execs the command via `sh -c` from the workspace dir with no LLM. Under `entrypoint`, exit 0 completes the run, stdout is the run output, and `model_id` must be unset. Entrypoint paths are workspace-relative skill scripts. Prefix the interpreter for the build's script language, as in `python3 ./.agents/skills/<skill>/resources/script.py`, `node ./.agents/skills/<skill>/resources/script.js`, or `sh ./.agents/skills/<skill>/resources/script.sh`. |
...
Every step runs under four runner rules. The platform appends each rule that applies to the prompt of an `instruction` task. An `entrypoint` script receives no prompt, so the skill writes each rule into the script itself.

- Durable output is optional. The runner sets `$EXO_OUTPUT_DIR` on workflow runs only, so a standalone `trigger_task_run` has no output directory. When a step writes nothing there, the runner saves the step's run output as `output.txt`. That is the final message of an `instruction` task, or the stdout tail of an `entrypoint`. Write files only when a step produces more than its run output. The step's skill then names each file, with its exact path. Use `.md` for a report a person reads and `.json` for a result a machine reads. An `instruction` task writes nothing there unless its skill names the file. Only regular files are captured, and the whole capture is dropped above 32 MiB or 4096 files. Everything else in the working directory is discarded when the run ends.
- Metrics are emitted with `exo-metric emit key=value [key=value ...]`, which is on PATH on every worker. Values are numeric, and a later emit of the same key overwrites the earlier value. The runner silently drops any key that is not a `task_metric` on the emitting step's task. A task fails with `required metrics not emitted` when it skips a key defined on it. A task with no defined keys emits nothing and still passes.
- An `instruction` task must run `exo-done` when it finishes, or the run fails. An `entrypoint` task completes on exit code 0 instead.
- Memory is a SQLite database at `$EXO_AGENT_MEMORY_PATH`. It persists across runs only when the workflow sets `memory_enabled: true`, and it is keyed to that workflow. With `memory_enabled` on the task alone, the database starts empty on every run and is not saved. The runner saves the database after each step that completes, so a failed step's writes are lost. A database over 32 MiB is not saved. Scripts open the path directly, with Python's `sqlite3` module or the `sqlite3` command in `base_tools`. The `instruction` prompt describes memory only when it is enabled.

Each step's emitted values merge into the workflow run's `workflow_metrics`, which outcomes and dashboards read. The platform does not require a workflow to emit anything. The build owns that rule: every workflow emits its outcome metrics. Emit each metric from the one step that owns it, and define its key on that step's task. Emit from a script when the value is deterministic, and tell the model in the skill not to emit that key.
