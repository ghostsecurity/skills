...
- Moving work into a script often adds a command line tool. Before proposing it, resolve the tool by the Worker capabilities procedure in common.md, and carry the result into the proposal: the capability to add under `metadata.requires`, and whether one pool still provides everything the workflow requires once it is added. A change that needs a platform admin to alter a pool is not something this loop can apply and rerun, so say so at the first gate. Where `addons_available` is false, propose only tools the workers already carry.
